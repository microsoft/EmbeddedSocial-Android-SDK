...
public class SlidingTabLayout extends HorizontalScrollView {
	...
	private void populateTabStrip() {
		...
		for (int i = 0; i < adapter.getCount(); i++) {
			...
			tabTitleView.setTextColor(getTabTextColor(i));
	...
	private int getTabTextColor(int position) {
		return mViewPager.getCurrentItem() == position ? activeTextColor : notActiveTextColor;
	}
		...
		public void onPageSelected(int position) {
			...
			for (int index = 0; index < mTabStrip.getChildCount(); index++) {
				...
				if (child instanceof TextView) {
					((TextView) child).setTextColor(getTabTextColor(index));
				}
			}
